...
template <typename T>
void WriteNblock(std::string &filename, const int max_node_id,
                 const NDArray<const int, 1> node_id_arr,
                 const NDArray<const double, 2> nodes_arr,
                 const NDArray<const double, 2> angles_arr, int sig_digits,
                 std::string &mode) {
...
void WriteEblock(
    ...
    const NDArray<const int, 1>
        typenum_arr, // ANSYS type number (e.g. 187 for SOLID187)
...
template <typename T>
void ResetMidside(NDArray<const uint8_t, 1> celltypes_arr,
