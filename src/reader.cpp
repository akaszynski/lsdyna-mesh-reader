...
// Fast string to integer converter
...
// Reads fortran formatted float formats in the form of
// "3.7826539829200E+00"
// "1.0000000000000E-001"
//
// fltsz : Number of characters to read in a floating point number
static inline int strtod(char *raw, int fltsz, double *arr) {
  char *end = raw + fltsz;
  double sign = 1;
...
int ReadNBlockMemMap(MemoryMappedFile &memmap, int *nnum, double *nodes,
                     double *node_angles, const int nnodes,
                     std::array<int, 3> d_size, const int f_size) {
...
public:
  ...
  Deck(const std::string &fname, bool readParams = false, bool dbg = false,
       bool readEblock = true)
      : filename(fname), read_elements(readEblock),
        debug(dbg) memmap(fname.c_str()) {
  ...
  //     // if (token.compare(0, 2, "ET") != 0 && token.compare(0, 2, "et") !=
  //     0) {
  //     //     return;
  //     // }
  ...
  //         // We only care about the first two integers in the block, the rest
  //         are keyopts if (!(iss >> elem_id >> elem_type_id)) {
  //             // allow a soft return
  //             std::cerr << "Failed to read ETBLOCK entry at line: " <<
  //             memmap.line
  //                       << std::endl;
  //             return;
  //         }
  ...
  //     std::transform(
  //         memmap.line.begin(), memmap.line.end(), memmap.line.begin(),
  //         [](unsigned char c) {
  ...
  //     // we have to allocate memory for the maximum size since we don't know
  //     that a priori int *elem_data = AllocateArray<int>(n_elem * 30);
  //     elem_off_arr = MakeNDArray<int, 1>({n_elem + 1});
  //     int elem_sz = ReadEBlockMemMap(memmap, elem_off_arr.data(), elem_data,
  //     n_elem);

  //     // wrap the raw data but limit it to the size of the number of elements
  //     read
  //     // Note: This is faster than reallocating a new array but will use more
  //     memory elem_arr = WrapNDarray<int, 1>(elem_data, {elem_sz});

  //     // must mark eblock is read since we can only read one eblock per
  //     archive file eblock_is_read = true;
  // }
  ...
  //     } catch (const std::invalid_argument &) {
  //         // soft error
  //         std::cerr << "Invalid format in KEYOPT line for elem_num:" <<
  //         memmap.line
  //                   << std::endl;
  //         return;
  //     }
  ...
  //         } catch (const std::invalid_argument &) {
  //             // soft error
  //             std::cerr << "Invalid format in KEYOPT line:" << memmap.line <<
  //             std::endl; return;
  //         }
  //     }
  ...
  //         nset = std::stoi(token);
  //     } catch (...) {
  //         std::cerr << "Error RLBLOCK line when reading nset:" << memmap.line
  //         << std::endl; return;
  //     }
  //     try {
  //         std::getline(iss, token, ',');
  ...
  //     } catch (...) {
  //         std::cerr << "Error RLBLOCK line when reading maxset:" <<
  //         memmap.line
  //                   << std::endl;
  //         return;
  //     }
  ...
  //     } catch (...) {
  //         std::cerr << "Error RLBLOCK line when reading maxitems:" <<
  //         memmap.line
  //                   << std::endl;
  //         return;
  //     }
  ...
  //     } catch (...) {
  //         std::cerr << "Error RLBLOCK line when reading nperline:" <<
  //         memmap.line
  //                   << std::endl;
  //         return;
  //     }
  ...
  //         //        2       6  1.00000000     0.566900000E-07  0.00000000
  //         0.000... memmap.read_line(); std::vector<double> rcon;

  //         // real constant number
  //         try {
  //             rnum.push_back(std::stoi(memmap.line.substr(0, 8)));
  //         } catch (...) {
  //             std::cerr << "Failed to read real constant ID when reading:" <<
  //             memmap.line
  //                       << std::endl;
  //             return;
  //         }
  ...
  //         if (ncon > 6) {
  //             for (int j = 0; j < 6; j++) {
  //                 rcon.push_back(std::stod(memmap.line.substr(16 + 16 * j,
  //                 16))); ncon--;
  //             }
  ...
  //                     rcon.push_back(std::stod(memmap.line.substr(16 * j,
  //                     16))); ncon--;
  //                 }
  //                 if (ncon > 0) {
  //                     memmap.read_line();
  ...
  //     // Sometimes, DAT files contains multiple node blocks. Read only the
  //     first block. if (nblock_is_read) {
  //         return;
  //     }
  //     nblock_start = pos;
  ...
  //         n_nodes = std::stoi(memmap.line.substr(memmap.line.rfind(',') +
  //         1));
  //     } catch (...) {
  //         std::cerr << "Failed to read number of nodes when reading:" <<
  //         memmap.line
  //                   << std::endl;
  //         return;
  //     }
  ...
  //     node_angles_arr = WrapNDarray<double, 2>(node_angles_data, {n_nodes,
  //     3});
  //     // std::cout << memmap.tellg() << std::endl;
  ...
  //     // Must mark nblock is read since we can only read one nblock per
  //     archive file. nblock_is_read = true; if (debug) {
  //         // std::cout << "Last line is: " << memmap.line << std::endl;
  //         std::cout << "NBLOCK complete, read " << n_nodes << " nodes." <<
  //         std::endl;
  //     }
  // }
  ...
  //         std::cerr << "Poor formatting of CMBLOCK: " << memmap.line <<
  //         std::endl; return;
  //     }

  //     int ncomp;
  //     try {
  //         ncomp = std::stoi(split_line[3].substr(0,
  //         split_line[3].find('!')).c_str());
  //     } catch (...) {
  //         std::cerr << "Poor formatting of CMBLOCK: " << memmap.line <<
  //         std::endl; return;
  //     }
  ...
  //     } catch (...) {
  //         std::cerr << "Failed to read number of integers per line in
  //         CMBLOCK" << std::endl; return;
  //     }
  ...
  //         }
  //         try {
  //             component[i] = std::stoi(memmap.line.substr(isz * (i % nblock),
  //             isz));
  //         } catch (...) {
  //             std::cerr << "Failed to parse integer from CMBLOCK line: " <<
  //             memmap.line
  //                       << std::endl;
  //             return;
  //         }
  ...
  // Read the LS-DYNA deck by reading the start of each line and checking if
  // it matches a keyword. For example, *NODE denotes a node card.
  // Note: Cards appear to start with the * char.
  void Read() {
  ...
  //     NDArray<int64_t, 1> cells_arr = WrapNDarray<int64_t, 1>(cell_data,
  //     {loc}); return nb::make_tuple(offset_arr, celltypes_arr, cells_arr);
  // }
...
nb::tuple ConvertToVTK(NDArray<int, 1> elem_arr, NDArray<int, 1> elem_off_arr,
                       NDArray<int, 1> type_ref, NDArray<int, 1> nnum_arr) {
  ...
  int loc = ans_to_vtk(n_elem, elem_arr.data(), elem_off_arr.data(),
                       type_ref.data(), n_nodes, nnum_arr.data(),
                       offset_arr.data(), cell_data, celltypes_arr.data());
...
NB_MODULE(_reader, m) {
  ...
  nb::class_<Deck>(m, "Deck")
      .def(nb::init<const std::string &, bool, bool, bool>(), "fname"_a,
           "read_params"_a = false, "debug"_a = false, "read_eblock"_a = true,
           "This class represents a LS-DYNA keyword deck.")
      ...
      .def_ro("node_angles", &Archive::node_angles_arr,
